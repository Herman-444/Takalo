    ...
    <main class="dashboard-main">
        <div class="container">
            ...
            <div class="page-header">
                <h1 class="page-title">Modifier l'objet</h1>
                <a href="/user/objets" class="btn btn-secondary">← Retour à mes objets</a>
            </div>

            <?php if (!empty($_SESSION['success_message'])): ?>
                <div class="alert alert-success"><?= htmlspecialchars($_SESSION['success_message'], ENT_QUOTES, 'UTF-8') ?></div>
                <?php unset($_SESSION['success_message']); ?>
            <?php endif; ?>

            <?php if (!empty($_SESSION['error_message'])): ?>
                <div class="alert alert-error"><?= htmlspecialchars($_SESSION['error_message'], ENT_QUOTES, 'UTF-8') ?></div>
                ...
                <div class="images-grid">
                    <?php foreach ($images as $image): ?>
                        <div class="image-item">
                            <img src="/images/<?= htmlspecialchars($image['image_path'], ENT_QUOTES, 'UTF-8') ?>" alt="Image de l'objet">
                            <form method="POST" action="/user/objets/image/delete">
                                <input type="hidden" name="image_id" value="<?= $image['id'] ?>">
                                <input type="hidden" name="objet_id" value="<?= $objet['id'] ?>">
                                <button type="submit" class="btn-delete-image" onclick="return confirm('Supprimer cette image ?');">✕</button>
                            </form>
                        </div>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="form-container">
                <form method="POST" action="/user/objets/update" class="admin-form" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="title" name="title" value="<?= htmlspecialchars($objet['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="Ex: Ballon de football, Vélo, Livre..." required>
                    </div>

                    <div class="form-group">
                        <label for="description">Description</label>
                        <textarea id="description" name="description" placeholder="Description détaillée de l'objet..." rows="4"><?= htmlspecialchars($objet['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="prix_estime" name="prix_estime" value="<?= htmlspecialchars($objet['prix_estime'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="Ex: 25.00" step="0.01" min="0">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="qtt">Quantité</label>
                        <input type="number" id="qtt" name="qtt" value="<?= htmlspecialchars($objet['qtt'] ?? 1, ENT_QUOTES, 'UTF-8') ?>" min="1">
                    </div>
